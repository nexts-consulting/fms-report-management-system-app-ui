...
interface UseCheckinPhotoUploadOptions {
  user: KeycloakUser;
...
/**
 * Hook to manage photo upload for check-in process
 */
export const useCheckinPhotoUpload = ({
  user,
  onUploadSuccess,
  onUploadError,
}: UseCheckinPhotoUploadOptions) => {
    ...
    async (file: File) => {
      ...
      try {
        ...
        const uploadResult = await uploadFileToCloud(
          ...
          {
            provider: "firebase",
            path: `${buildPath("/uploads")}/attendance/checkin/${user.id}`,
            storage: firebaseService.storage,
            generateFileName: (file) => {
              const timestamp = Date.now();
              const extension = file.name.split(".").pop() || "jpg";
              return `checkin_${timestamp}.${extension}`;
            },
          },
          (progress) => {

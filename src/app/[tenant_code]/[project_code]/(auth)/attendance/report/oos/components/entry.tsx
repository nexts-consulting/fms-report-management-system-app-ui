...
import { useMutationCreateOOSReport } from "@/services/api/reports/oos";
...
export const Entry = () => {
  ...
  const {
    ...
    defaultValues: React.useMemo(() => {
      if (!currentAttendance?.oosReport?.data) {
        return { items: {} };
      }

      // Transform array data to object format
      const items = currentAttendance.oosReport.data.reduce(
        (acc, item) => ({
          ...acc,
          [item.sku]: {
            pcs: item.pcs,
  ...
  const createOOSReportMutation = useMutationCreateOOSReport({
    config: {
      onSuccess: () => {
        ...
        if (submitStatusNotifyIdRef.current) {
          notification.update(submitStatusNotifyIdRef.current, {
            ...
            description: "Báo cáo OOS đã được lưu thành công",
            options: {
              duration: 10000,
            },
          });

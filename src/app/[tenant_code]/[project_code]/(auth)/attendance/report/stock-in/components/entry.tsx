...
import { useMutationCreateStockInReport } from "@/services/api/reports/stock-in-report";
...
export const Entry = () => {
  ...
  const {
    ...
    defaultValues: React.useMemo(() => {
      if (!currentAttendance?.stockInReport?.data) {
        return { items: {} };
      }

      // Transform array data to object format
      const items = currentAttendance.stockInReport.data.reduce(
        (acc, item) => ({
          ...acc,
          [item.sku]: {
            pcs: item.pcs,
      ...
      return { items };
    }, [currentAttendance?.stockInReport]),
  });
  ...
  const createOOSReportMutation = useMutationCreateStockInReport({
    config: {
      onSuccess: () => {
        ...
        if (submitStatusNotifyIdRef.current) {
          notification.update(submitStatusNotifyIdRef.current, {
            ...
            description: "Báo cáo Tồn đầu ca đã được lưu thành công",
            options: {
              duration: 10000,
            },
          });

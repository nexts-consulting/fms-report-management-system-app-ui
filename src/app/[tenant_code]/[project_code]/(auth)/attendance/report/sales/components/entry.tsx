...
import { useMutationCreateSalesReport } from "@/services/api/reports/sale";
...
export const Entry = () => {
  ...
  const {
    ...
    defaultValues: React.useMemo(() => {
      if (!currentAttendance?.saleReport?.data) {
        return { items: {} };
      }

      // Transform array data to object format
      const items = currentAttendance.saleReport.data.reduce(
        (acc, item) => ({
          ...acc,
          [item.sku]: {
            pcs: item.pcs,
      ...
      return { items };
    }, [currentAttendance?.saleReport]),
  });

  const isReportedSale = React.useMemo(() => {
    return currentAttendance?.saleReport?.id !== null;
  }, [currentAttendance]);
  ...
  const createSalesReportMutation = useMutationCreateSalesReport({
    config: {
      onSuccess: () => {
        ...
        if (submitStatusNotifyIdRef.current) {
          notification.update(submitStatusNotifyIdRef.current, {
            ...
            description: "Báo cáo bán hàng đã được lưu thành công",
            options: {
              duration: 10000,
            },
          });
  ...
  const onFormSubmit = (data: FormValues) => {
    ...
    createSalesReportMutation.mutate({
  ...
  return (
    <>
      <LoadingOverlay active={createSalesReportMutation.isLoading} />
      ...
      <form onSubmit={handleSubmit(onFormSubmit, onFormError)} className="flex h-full flex-col">
        ...
        {Object.keys(groupedItems).length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 border-t bg-white p-4">
            <Button
              type="submit"
              disabled={createSalesReportMutation.isLoading}
              variant={"primary"}
              className="w-full"
              centered
            >
              {createSalesReportMutation.isLoading ? "Đang lưu báo cáo" : "Lưu báo cáo"}
            </Button>
          </div>
        )}
      </form>

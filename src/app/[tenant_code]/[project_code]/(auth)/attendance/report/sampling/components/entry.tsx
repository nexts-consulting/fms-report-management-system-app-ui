...
import { useMutationCreateSamplingReport } from "@/services/api/reports/sampling";
...
export const Entry = () => {
  ...
  const {
    ...
    defaultValues: React.useMemo(() => {
      if (!currentAttendance?.samplingReport?.data) {
        return { items: {} };
      }

      // Transform array data to object format
      const items = currentAttendance.samplingReport.data.reduce(
        (acc, item) => ({
          ...acc,
          [item.sku]: {
            pcs: item.pcs,
      ...
      return { items };
    }, [currentAttendance?.samplingReport]),
  });

  const isReportedSampling = React.useMemo(() => {
    return currentAttendance?.samplingReport?.id !== null;
  }, [currentAttendance]);
  ...
  const createSamplingReportMutation = useMutationCreateSamplingReport({
    config: {
      onSuccess: () => {
        ...
        if (submitStatusNotifyIdRef.current) {
          notification.update(submitStatusNotifyIdRef.current, {
            ...
            description: "Báo cáo sampling đã được lưu thành công",
            options: {
              duration: 10000,
            },
          });
  ...
  const onFormSubmit = (data: FormValues) => {
    ...
    createSamplingReportMutation.mutate({
  ...
  return (
    <>
      <LoadingOverlay active={createSamplingReportMutation.isLoading} />

      <ScreenHeader
        title="Báo cáo sampling"
      ...
      <form onSubmit={handleSubmit(onFormSubmit, onFormError)} className="flex h-full flex-col">
        ...
        {Object.keys(groupedItems).length > 0 && (
          <div className="fixed bottom-0 left-0 right-0 border-t bg-white p-4">
            <Button
              type="submit"
              disabled={createSamplingReportMutation.isLoading}
              variant={"primary"}
              className="w-full"
              centered
            >
              {createSamplingReportMutation.isLoading ? "Đang lưu báo cáo" : "Lưu báo cáo"}
            </Button>
          </div>
        )}
      </form>

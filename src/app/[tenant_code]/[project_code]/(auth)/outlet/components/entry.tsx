...
import { useQueryLocationByAdminDivision } from "@/services/api/location/list-by-admin-division";
import { useQueryAdminDivisionList } from "@/services/api/admin-division/list";
import {
  IAdminDivision,
...
export const Entry = () => {
  ...
  const maxLevel = React.useMemo(() => {
    ...
    return Math.max(...adminDivisionListQuery.data.data.map((d) => d.level));
  }, [adminDivisionListQuery.data]);
  ...
  const getLevelOptions = React.useCallback(
    (level: number, parentId: number | null): Array<{ key: string; label: string; value: IAdminDivision }> => {
      if (level === 1) {
        ...
        return adminDivisionTree.map((division, index) => ({
      ...
      <div className="px-4">
        ...
        {isDisplayLocationList && (
          <div className="mt-4 w-full bg-white px-4 py-12">
            <div>
              {locationListQuery.data?.data.map((location, index) => (
                <LocationCard

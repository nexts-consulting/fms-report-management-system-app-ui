...
type HttpRequestAuthRefreshParams = {
  refreshToken: string;
  tenantCode: string;
 ...
 */
const httpRequestAuthRefresh = async (
  params: HttpRequestAuthRefreshParams,
): Promise<AuthRefreshResponseData> => {
  const { refreshToken, tenantCode, keycloakBaseUrl } = params;
...
const useMutationAuthRefresh = ({ config }: MutationOptions = {}) => {
  return useMutation({
...
export { httpRequestAuthRefresh, useMutationAuthRefresh };
